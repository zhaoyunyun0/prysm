...
func TestVotes_CanFindHead(t *testing.T) {
	...
	//             |
	//             4 <- head
	state, blkRoot, err = prepareForkchoiceState(context.Background(), f, 0, indexToHash(4), indexToHash(3), params.BeaconConfig().ZeroHash, 1, 1)
	require.NoError(t, err)
	require.NoError(t, f.InsertNode(ctx, state, blkRoot))
	...
	state, blkRoot, err = prepareForkchoiceState(context.Background(), f, 2*params.BeaconConfig().SlotsPerEpoch, indexToHash(5), indexToHash(4), params.BeaconConfig().ZeroHash, 2, 2)
	require.NoError(t, err)
	require.NoError(t, f.InsertNode(ctx, state, blkRoot))
	...
	state, blkRoot, err = prepareForkchoiceState(context.Background(), f, 0, indexToHash(6), indexToHash(4), params.BeaconConfig().ZeroHash, 3, 2)
	require.NoError(t, err)
	require.NoError(t, f.InsertNode(ctx, state, blkRoot))
	r, err = f.Head(context.Background())
	require.NoError(t, err)
	...
	//           |
	//           8
	state, blkRoot, err = prepareForkchoiceState(context.Background(), f, 0, indexToHash(7), indexToHash(5), params.BeaconConfig().ZeroHash, 2, 2)
	require.NoError(t, err)
	require.NoError(t, f.InsertNode(ctx, state, blkRoot))
	state, blkRoot, err = prepareForkchoiceState(context.Background(), f, 0, indexToHash(8), indexToHash(7), params.BeaconConfig().ZeroHash, 2, 2)
	require.NoError(t, err)
	require.NoError(t, f.InsertNode(ctx, state, blkRoot))
	r, err = f.Head(context.Background())
	require.NoError(t, err)
	...
	//           |
	//           10 <- head
	state, blkRoot, err = prepareForkchoiceState(context.Background(), f, 0, indexToHash(10), indexToHash(8), params.BeaconConfig().ZeroHash, 3, 2)
	require.NoError(t, err)
	require.NoError(t, f.InsertNode(ctx, state, blkRoot))
	r, err = f.Head(context.Background())
	require.NoError(t, err)
	...
	//           / \
	//	    9  10
	state, blkRoot, err = prepareForkchoiceState(context.Background(), f, 0, indexToHash(9), indexToHash(8), params.BeaconConfig().ZeroHash, 3, 2)
	require.NoError(t, err)
	require.NoError(t, f.InsertNode(ctx, state, blkRoot))
	...
	//        |
	// head-> 11
	state, blkRoot, err = prepareForkchoiceState(context.Background(), f, 0, indexToHash(11), indexToHash(10), params.BeaconConfig().ZeroHash, 3, 2)
	require.NoError(t, err)
	require.NoError(t, f.InsertNode(ctx, state, blkRoot))
